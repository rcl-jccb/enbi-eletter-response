...
def get_data_df(rootpath, data_type='taxonomy', unwanted_strs = [], wanted_strs=[], taxonomy_level='species'):
    if data_type == 'taxonomy':
        ...
        data_file = os.path.join(rootpath,'pathabundance_relab.tsv')
        ...
        for unwanted_str in unwanted_strs:
            filtered_df = filtered_df[
                ~filtered_df.index.str.contains(unwanted_str)
            ]
        
        if len(wanted_strs) != 0:
            matching_indices = []
            for wanted_str in wanted_strs:
                # Find matching indices for the current wanted string
                matches = filtered_df.index[filtered_df.index.str.contains(wanted_str)]
            ...
            # Create the new DataFrame with only the matching rows
            filtered_df = df.loc[matching_indices]
        
       
        '''
        # Columns almost sum up to 1, with an error of 10^-5 or so.
        print(filtered_df)
        column_sums = filtered_df.sum(axis=0)
        columns_to_print = column_sums[column_sums != 1]

        # Print the column names and their sum
        for col, col_sum in columns_to_print.items():
            print(f"Column: {col}, Sum of rows: {col_sum}")
        
        '''
    elif data_type=='enzyme':
        data_file = os.path.join(rootpath,'ecs_relab.biom')
        with open(data_file, 'r') as file:
            biom_data = json.load(file)
        ...
        # With that, all columns sum up to 1 apart from 2 samples that sum to 0 so they will not count once we normalize and clean the nans
    elif data_type=='substance':
        data_file = os.path.join(rootpath,'metabolomics.biom')
        with open(data_file, 'r') as file:
            biom_data = json.load(file)
        ...
        print(f'The data type must be in [pathway, taxonomy, enzyme, substance] you introduced: {data_type}')
...
def plot_diversity_distributions(image_dir, participant_dict, aux_data_dict=None, data_type = 'Species', metric='Shannon'):
    ...
    ax = plt.gca()
    plt.boxplot([healthy_metric, unhealthy_metric], labels=['Healthy', 'Unhealthy'], showfliers=False)
    plt.ylabel(ylabel)
...
#rootpath_results = os.path.join(gut_dir,'results', 'real_data','IBD_MDB')
#rootpath_img = os.path.join(gut_dir,'images', 'real_data','IBD_MDB')
#metadata_dict = get_metadata_dict(rootpath_data)
#data_df_pathway = get_data_df(rootpath_data, data_type = 'pathway')
#data_df_pathway.index.to_series().to_csv("pathways_filtered_list.txt", index=False, header=False)
...
#data_df_substances = get_data_df(rootpath_data, data_type = 'substance')
...
#full_data_dict_substances = get_full_data_dict(metadata_dict, data_df_substances)
#plot_diversity_distributions(rootpath_img, full_data_dict_taxonomy, aux_data_dict = None, data_type='Species', metric='Shannon')
#plot_diversity_distributions(rootpath_img, full_data_dict_enzymes, aux_data_dict = None, data_type='Enzymes', metric='Number')
#plot_diversity_distributions(rootpath_img, full_data_dict_substances, aux_data_dict = None, data_type='Substances', metric='Shannon')
#plot_diversity_distributions(rootpath_img, full_data_dict_substances, aux_data_dict = None, data_type='Substances', metric='Number')
...
rootpath_img = os.path.join(gut_dir,'images', 'real_data','IBD_MDB', 'longitudinal_analyses', f'{wanted_n_species}_{t_points_threshold}_{error_threshold}_{n_bagging}')

unwanted_strs = ['genesis', 'synthesis', 'salvage', 'assimilation', 'elongation', 'interconversion', 'Calvin-Benson', 'Methylerythritol',
                 '(non-oxidative branch)', 'reductive', 'transformations', 'gamma;-glutamyl', 'urea cycle', 'NAD/NADH', 'pyridine nucleotide cycling',
                 'reduction', 'S-adenosyl-L-methionine cycle I', 'mevalonate pathway', 'Kdo transfer to lipid IVA III', 'hydrogen production VIII',
                 'phosphatidylcholine acyl editing', 'tRNA methylation', 'protein N-glycosylation', 'phosphatidate metabolism, as a signaling molecule',
                 'pyrimidine deoxyribonucleotide phosphorylation', 'NAD/NADP', 'formation', 'phospholipid remodeling', 'monoacylglycerol metabolism',
                 'anhydromuropeptides recycling', 'polymyxin resistance', 'tRNA', 'aspartate superpathway', 'superpathway of L-threonine metabolism']
wanted_strs=['degradation', 'fermentation', 'lysis', 'oxidation', 'TCA', 'shunt', 'Entner', 'Glyoxylate', 'methylerythritol phosphate pathway']
metadata_dict = get_metadata_dict(rootpath_data)
#data_df_enzymes = get_data_df(rootpath_data, data_type = 'enzyme')
#data_df_pathway = get_data_df(rootpath_data, data_type = 'pathway', unwanted_strs=unwanted_strs)
#data_df_pathway = get_data_df(rootpath_data, data_type = 'pathway', unwanted_strs=[], wanted_strs=wanted_strs)
data_df_taxonomy = get_data_df(rootpath_data, data_type = 'taxonomy')
full_data_dict_taxonomy = get_full_data_dict(metadata_dict, data_df_taxonomy)
#full_data_dict_pathway = get_full_data_dict(metadata_dict, data_df_pathway)
#full_data_dict_enzymes = get_full_data_dict(metadata_dict, data_df_enzymes)
#print(data_df_pathway)
#plot_diversity_distributions(rootpath_img, full_data_dict_pathway, aux_data_dict = None, data_type='Filtered_Pathways_unwanted',
#                             metric='Number')

interaction_matrices_median_dict, interaction_matrices_avg_dict = get_interaction_mat_dict_limits(rootpath_results, full_data_dict_taxonomy, 
                                                                                                  wanted_n_species, t_points_threshold, 
                                                                                                  error_threshold, n_bagging, overwrite=False)
stats_dict_median, stats_dict_avg = get_diagnosis_stats_dict(interaction_matrices_median_dict, interaction_matrices_avg_dict, full_data_dict_taxonomy)
